...
- [x] Pilot with the smallest/least in-flight meta-based project; `real-time-computing` (MIT Press production constraints, hardware versioning) goes **last**. *(All four meta books migrated: math 2026-06-10 (344pp), engineering-computing + system-dynamics 2026-06-11 (282pp/446pp), rtc 2026-06-12 (419pp, 0 missing citations, versioning plugin active ts:T1/ds:D1). Each: schema-v2 artifact + print PDF + preview green.)*
...
- [x] Hardware versioning (`versions.json` → inheritance flattening → conditional prose + parts lists): port **only when the migrating book needs it**, as a Parody plugin/filter, not core. *(2026-06-11: `parody/plugins/versioning.py` — first plugin; named tracks (rtc ts/ds two-track and single-track books are both just config; 2nd-ed = `tracks: {ts: T2, ds: D2}`), variant-inheritance flattening, `[]{.ts}`/`[]{.tsicon}`/`[]{.T1-…}` span substitution in both pipelines, unresolved params warn (surfaced 4 stale rtc refs meta hid).)* **Parts-list / versions-lister DONE** *(2026-06-14: `parody/plugins/partslist.py`. Discovery: the rtc lab appendices were already migrated and print-complete; the real gap was that meta's versions-lister generates the general hardware catalog two ways — LaTeX (print) + HTML `<details>` markdown (web) — and migration captured only the print tex, so the general catalog (labels wp, a6, y6, lf, wi + component details) was missing from the artifact. Ported the markdown branch as a **pipeline-aware** `make_transform` (new: `content_transforms(meta, dir, target)` passes target to transforms that accept it; build.py=artifact, latex.py=print). The transform expands a `[]{.parts-list .ts|.ds}` marker into the `<details>` catalog (built from versions.yaml for the active tracks, dynamic per edition) for the artifact and drops it for print (print keeps its migration tex). Migrator `_inject_partslist_marker` emits the marker after the localized versions-list `\input` + enables `parts_list` mirroring the versioning tracks. Live on rtc: artifact gained general-target-system-T1 + target-computer(a6)/ui/electromechanical/prototyping anchors; print unchanged (0 undefined, 469pp). This is the last book-json-era deferred item — meta's common/ is now fully ported for rtc.)*
- [x] **Apocrypha plugin** (`book-json/apocrypha.json` → online-only section metadata for website TOCs): port as the second opt-in plugin. *(2026-06-14: `parody/plugins/apocrypha.py` — introduces a second plugin-hook kind, `make_artifact_hook(config, dir) -> (artifact)->artifact`, a post-processor for plugins that contribute book-level metadata rather than rewriting prose; `plugins/__init__` gains `artifact_hooks`/`apply_artifact_hooks`, both lookups hasattr-guarded. `normalize_entries` flattens the meta nested `{hash:{subhash:fields}}` and the parody list shape (idempotent, drops empty-hash); the hook emits the **online-only** subset (drops entries whose hash is an in-book section/anchor, keeps versioned variants carrying a subhash). schema-v2 gains optional top-level `apocrypha`; migrator `copy_apocrypha()` writes `apocrypha.yaml` + sets `apocrypha: true`. Live on all four books: rtc 961→687, systems 669→370, math 288→156, ec 187→73 online-only; all artifacts validate. Videos.json (the sibling book-json file) still not ported.)*
- [x] **Book-metadata plugin** (`book-defs.json` → editions/bibliographic metadata): third opt-in plugin, second of the artifact-hook kind. *(2026-06-14: `parody/plugins/bookmeta.py` — `normalize_book` flattens the meta shape (`book-name`, `url-companion`, `editions:{id:{…}}`) and the parody shape (`name`, `companion_url`, `editions:[…]`); drops meta-internal keys (`json-file-*`, apocrypha pointer) and placeholders (TBD, all-zero ISBNs); ec's `v`→edition. Small curated data so it lives **inline** in parody.yaml under `book:` (the config value IS the metadata, no sidecar). schema-v2 gains optional top-level `book`. Migrator `book_metadata()` writes the block from book-defs.json **guarded by a title-prefix check** — math & systems carry a copied placeholder book-defs ("Engineering Computing for Design" + dummy ISBNs), correctly skipped. Live on rtc (4 editions, real MIT Press ISBN on the published one, per-edition v-ts/v-ds track bindings preserved) and ec (1). Per-edition track bindings connect to the versioning plugin.)*
- [x] **Videos plugin** (`book-json/videos.json` → lecture-video playlist links): fourth opt-in plugin, third of the artifact-hook kind. *(2026-06-14: `parody/plugins/videos.py` — `normalize_videos` flattens the meta shape (`{edition:{playlist,indices:{hash:idx|[idx]}}}`) and the parody list shape, preserving playlist order, widening ints→lists (idempotent). Sidecar `videos.yaml` (`videos: true`); hook emits only links whose hash is a real section of this build (in-book or apocrypha), dropping dangling. Registered AFTER apocrypha so the hook reads `artifact["apocrypha"]`. schema-v2 gains optional top-level `videos`. Migrator `copy_videos()` guarded by a **hash-overlap check** (<25% present → skip): math and systems ship a byte-identical copied videos.json; the overlap check correctly attributes it to **math** (86% overlap, 37 links) and skips **systems** (22%, stale copy). Live: rtc 20 links + math 37. This completes the book-json trio (book-defs + apocrypha + videos); meta's `book-json/` is now fully ported.)*
...
**Homepage-django v2 import — DONE** *(2026-06-14, homepage-django branch `parody-v2-textbook-import`): the notebook importer capped at schema v1, rejecting the migrated textbooks. Extended it to schema v2 — `NotebookChapter` gained `hash`+`appendix`, `NotebookSection` gained `hash`, `Notebook` gained `book_metadata`/`videos`/`apocrypha` JSONFields (migration 0075); `import_notebooks` reads them (v1 artifacts unchanged). Surfaced + fixed a parody bug: the artifact builder dropped the chapter `appendix` flag (parody `aa1b882`). Verified end-to-end: rtc imports as a first-class notebook (18 ch / 120 sec, all hashed; 9 appendix chapters; 4 editions w/ real ISBN; 686 apocrypha) with the 13 existing notebooks byte-identical (regression clean) and the teaching suite + 2 new tests green. Remaining for full delivery: release the rtc content repo + pin its artifact via `teaching/content-manifest.json` (the per-notebook flow in homepage's NOTEBOOKS_SPLIT_PLAN.md Phase 4). Optional UI follow-up: surface editions/ISBN, video playlists, and the apocrypha TOC. This is the seed plan's stated point — books as first-class notebooks-UI citizens.)*

**rtcbook.org partial web publication — mechanism DONE; renderer next** *(2026-06-14): MIT Press only permits the PARTIAL rtc book public on rtcbook.org — the TS/DS-specific online-only sections (versioned, not in the print book) + per-section online-resources addenda, never the full copyrighted book. Built the renderer-agnostic artifact mechanism (parody commit 043895b): sections carry `online_only` + `online_resources` (schema v2); `parody build --online-only` emits the partial artifact (online-only sections in full + online-resources as body-stripped resource pages, MIT-safe); migrator imports meta's online-resources-editable into `<slug>.online.md`. Validated on rtc (3 online-only sections; 117 copyrighted excluded). RENDERER DECISION (revised from seed-plan's original static-preview note): a **dedicated, fresh, minimal Django book-host** (NOT static, NOT the grades monolith) that imports ONLY the partial artifact and renders the Django-flavored html with real template tags — decoupled + MIT-safe + reusable for standalone book sites. That book-host is the next build (task #267).)* **Django book-host BUILT** *(2026-06-14, new repo `~/parody-book-host`, task #267): a minimal standalone Django site that imports ONE parody artifact and renders it; fed the partial (`--online-only`) artifact, the full text never enters its DB. Renders parody's Django-template-flavored html NATIVELY (`book.templatetags.book_tags.render_book` resolves `{% media %}`/`{% cite %}`/etc. via the Django engine — no second lossy renderer). book app: Book/Chapter/Section models (online_only, online_resources, hash, appendix, book/videos/apocrypha), `import_artifact` (upsert by slug, idempotent), public index + section views, templates. 7 tests. Validated on the rtc partial: index shows title+ISBN; the online-only specific-T1 section renders with myRIO content, `{% media %}`→/media/, `<details>` intact, no raw tags. NOT yet deployed to rtcbook.org (DNS/hosting/media-staging = follow-up task #268).)*
